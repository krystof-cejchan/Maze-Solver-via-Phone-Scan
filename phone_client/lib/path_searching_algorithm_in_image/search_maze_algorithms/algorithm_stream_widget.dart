...
import '../support_classes/node.dart';

class AlgorithmStreamWidget extends StatefulWidget {
  final List<List<int>> grid;
  final Coordinates coordinates;
  final custom.Image originalBlackAndWhiteImg;
  const AlgorithmStreamWidget(
      {super.key,
      required this.grid,
      required this.coordinates,
      required this.originalBlackAndWhiteImg});

  @override
  State<StatefulWidget> createState() => _AlgorithmStreamState();
}

class _AlgorithmStreamState extends State<AlgorithmStreamWidget> {
  late final custom.Image copy = widget.originalBlackAndWhiteImg;
  final StreamController<ChangePixelAt> _stateController =
      StreamController<ChangePixelAt>();
  ...
  Widget build(BuildContext context) {
    return StreamBuilder(
      ...
      builder: (context, snapshot) {
        return Image.memory(widget.originalBlackAndWhiteImg.bytes);
      },
    );
  }
...
class ChangePixelAt {
  final int x, y;
  final img.ColorInt8 color;
  ChangePixelAt(this.x, this.y, this.color);
}
